...
impl<T: Trait> Module<T> {
	...
	fn insert_kitty(owner: T::AccountId, kitty_id: u32, _kitty: Kitty) {
		// 作业：完成方法
		// Kitties::insert(kitty_id,kitty);
		KittiesCount::put(kitty_id+1);

		let owner_id = Self::owned_kitties_count(&owner);

		<OwnedKitties<T>>::insert((&owner,owner_id),kitty_id);
		<OwnedKittiesCount<T>>::insert(&owner,owner_id +1);
	}
